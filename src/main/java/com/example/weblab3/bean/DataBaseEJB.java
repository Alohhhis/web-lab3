...
public class DataBaseEJB {
    ...
    private boolean isRInRange(float r) {
        for (float allowedR : R_VALUES) {
            if (allowedR == r) {
                return true; // r входит в допустимый диапазон
            }
        }
        return false; // r не входит в допустимый диапазон
    }
    ...
    public void addCanvas() {
        ...
        try {
            float x = Float.parseFloat(values.get("graphForm:hiddenX"));
            float y = Float.parseFloat(values.get("graphForm:hiddenY"));
            float r = Float.parseFloat(values.get("graphForm:hiddenR"));
            long time = System.nanoTime();
            if (x >= MIN_X && x <= MAX_X && y >= MIN_Y && y <= MAX_Y && isRInRange(r)) {
                boolean result = AreaChecker.isHit(x, y, r);
                LocalTime currentTime = LocalTime.now();
